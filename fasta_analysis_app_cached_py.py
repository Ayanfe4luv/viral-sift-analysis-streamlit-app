...
TRANSLATIONS = {
    "en": {
        ...
        "distribution_viewer_title": "📊 Data Visualizer",
    ...
    "ru": {
        ...
        "distribution_viewer_title": "📊 Визуализатор Данных",
        ...
        "enhanced_temporal_header": "Улучшенный Временной Фильтр",
        ...
        "footer_text": "FastaFlow - Инструмент Анализа Вирусных Геномов",
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("<h1 style='text-align: center; color: white;'>🧬 FastaFlow</h1>", unsafe_allow_html=True)
        ...
        if st.session_state.active_sequences:
            st.metric("🧬 Active Sequences", f"{len(st.session_state.active_sequences):,}")
            try:
        ...
        if st.button("🔄 Reset All Data", use_container_width=True, key="reset_all_sidebar"):
        ...
        if st.session_state.active_sequences:
            ...
            st.download_button(
                label="💾 Quick Export Active FASTA",
        ...
        if not st.session_state.all_files:
             ...
             st.markdown(f"""
                <div style='background: linear-gradient(135deg, #e0f2fe 0%, #ccfbf1 100%);
                    ...
                    <h3 style='color: #0c4a6e; border: none; margin-top: 0;'>👋 Welcome to FastaFlow!</h3>
                  ...
                  with st.spinner("🔄 Processing uploaded files..."):
                      ...
                      for idx, uploaded_file in enumerate(uploaded_files):
                          ...
                          if filename not in st.session_state.all_files:
                              try:
                                  ...
                                  if errors:
                                      has_errors = True
                                      st.warning(f"⚠️ Issues parsing {filename} (check log in Export tab). First error: {errors[0]}", icon="⚠️")
                      ...
                      if newly_loaded_count > 0:
                          ...
                          if not st.session_state.active_sequences:
                              st.info("💡 Go to 'Manage Datasets' to activate files for analysis.")
                      elif not has_errors:
        ...
        with col2:
             ...
             if st.button(T("download_url_btn"), use_container_width=True, key="url_download_button"):
                  if url_input and url_input.startswith(('http://', 'https://')):
                      with st.spinner(f"⏳ Downloading from {url_input[:50]}..."):
                          try:
                              ...
                              if content_string:
                                   ...
                                   if errors:
                                        st.warning(f"⚠️ Issues parsing {filename} from URL (check log). First: {errors[0]}", icon="⚠️")
                                        for err in errors[:5]:
    ...
    with tab_map["manage_tab"]:
        ...
        if not st.session_state.all_files:
            ...
            st.markdown("---")
            st.subheader("⚡ Actions on Selected Files")
            ...
            with action_cols[3]:
                ...
                if st.session_state.get("confirming_removal", False):
                    st.warning(f"⚠️ Are you sure you want to permanently remove {len(selected_files_now)} selected file(s) from this session?")
        ...
        if not st.session_state.active_sequences:
             ...
             st.markdown(f"""
                <div style='background: #fffbeb; padding: 25px; border-radius: 12px; border-left: 6px solid #f59e0b;'>
                    <h3 style='color: #92400e; border: none; margin-top: 0;'>⚠️ No Active Dataset</h3>
            ...
            # --- Data Visualizer ---
            with st.expander("🎨 Data Visualizer", expanded=False):
    ...
    with tab_map["export_tab"]:
        ...
        with col_exp1:
             ...
             if st.session_state.get('accession_list'):
                  ...
                  st.download_button(
                      label=f"⬇️ Download Extracted Accessions ({len(st.session_state.accession_list)} IDs)",
                      data=acc_data,
